...
public class DLinkedList {
	 ...
	 */
	/**
	 * This method sorts a doubly linked list using the insertion sort algorithm. It
	 * uses the values of the nodes to check if one is larger, and if yes the
	 * smaller value is swapped and the greater values are all shifted to the right.
	 * 
	 * It has a complexity of O(N^2) due to the nested loop.
	 * 
	 */
	public void insertionSort() {

		Node cur = null; // current node

		Node idx = null; // will hold the value of the node after the current node.

		int placeholder = 0; // placeholder for a node value.

		if (headNode == null) { // base case if list is empty
			return;
		}

		else {
			for (cur = headNode; cur.getNextNode() != null; cur = cur.getNextNode()) { // iterates through list with the
																						// current node

				for (idx = cur.getNextNode(); idx != null; idx = idx.getNextNode()) { // iterates through list with node
																						// after current node

					if (cur.getValue() > idx.getValue()) { // if idx value is less than current value it gets swapped

						placeholder = cur.getValue();

						cur.setValue(idx.getValue());

						idx.setValue(placeholder);
					}
				}
			}
		}
